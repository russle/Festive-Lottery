...
- **遠端遙控支援**：支援實體簡報器 (PPT Clicker)，讓主持人在舞台中央即可透過「下一頁」鍵控制抽獎流程。
...
- **遠端操控**：主持人手機掃描後開啟專屬控制頁面（未來計畫）或使用實體簡報器連動。

### 3. 遠端遙控操作 (主持人專用)
系統已針對實體簡報器 (如諾為 KNORVAY N26) 進行優化，只需將 USB 接收器插入筆電即可控制：
- **方向鍵 (上/下/左/右) / PageUp / PageDown / 空白鍵**：
  - **任何時候**：直接驅動抽獎流程（倒數 -> 停止滾動 -> 下一個獎項）。
  - **優點**：無需移動游標，隨按隨抽。
- **Tab 鍵**：
  - **功能**：正常移動頁面焦點（游標），便於手動選取特定按鈕。
- **Enter 鍵**：
  - **功能**：點擊目前游標所在的按鈕。
- **防誤觸保護**：當開啟「管理後台」或「設定面板」時，所有遙控按鍵會自動停用。
